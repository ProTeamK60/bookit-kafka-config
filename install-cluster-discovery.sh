...
sed -E 's/@service.namespace@/'"${DISCOVERY_SERVICE_NAME}.${NAMESPACE}"'/;s/@number.of.instances@/'"${TOTAL_INSTANCES}"'/' ./packages/scripts/opt/bookit/bin/configure-hosts.sh > /opt/bookit/bin/configure-hosts.sh
